...
struct
  ...
  let count_in_connected_component_fwd target compil network
      connected_component =
    network, VarMap.fold
      (fun vars id alg ->
         match vars with
  ...
  let count_in_connected_component_bwd target parameters compil network
    ...
    let cache = network.cache in
    let cache,(w,l) =
      I.equiv_class_of_pattern parameters
        compil
        cache
  ...
  let nocc_of_connected_component
    ...
    match network.sym_reduction with
    | Symmetries.Ground | Symmetries.Forward _ ->
      count_in_connected_component_fwd to_nocc
        compil network connected_component
    | Symmetries.Backward _ ->
  ...
  let rec convert_alg_expr parameter compil network alg =
    ...
    | Alg_expr.BIN_ALG_OP (op, arg1, arg2 ),loc ->
      let network, output1 =
        convert_alg_expr parameter compil network arg1
      in
      let network, output2 =
        convert_alg_expr parameter compil network arg2
      in
    ...
    |  (  Alg_expr.ALG_VAR _
      | Alg_expr.CONST _
       | Alg_expr.STATE_ALG_OP _),_ as a -> network, a
  ...
  let increment ~propagate_constants
    ...
    if is_zero x
    then
      Ode_loggers.associate ~propagate_constants ~init_mode ~comment string_of_var logger logger_buffer logger_err  (Ode_loggers_sig.Init x)
    else
  ...
  let export_dydt
    ...
    let () =
      ...
      then
        let () =
          List.iter
            (affect_var ~propagate_constants
      ...
      List.iter
        (fun ((reactants, products, token_vector, enriched_rule),nocc) ->
           ...
           let add_factor l  =
             ...
             then
               List.rev_map
                 (fun x ->
                    let nauto =
                      snd (species_of_species_id network x)
                    in
                    (x, nauto)) (List.rev l)
             else
           ...
           let () =
             ...
             then
               let rule_string =
                 Format.asprintf "%a" (I.print_rule_name ~compil)
                   enriched_rule.rule
               in
  ...
  let export_jac ~propagate_constants logger logger_err compil network split =
    ...
    | Loggers.Matlab | Loggers.Octave  ->
      ...
      let dep_var =
        List.fold_left
          (fun dep var ->
             affect_deriv_var ~propagate_constants
          ...
          (fun dep (rule,coef_list) ->
             List.fold_left
               (fun dep coef ->
                  match coef with
                  | R rate ->
                    affect_deriv_rate ~propagate_constants
                      dep_var
                      logger logger logger_err compil
                      rule rate network
                      dep
                  | S (n,rate) ->
                    affect_deriv_stoch ~propagate_constants
                      dep_var
                      logger logger logger_err compil
                      rule n rate network
                      dep)
  ...
  let export_network
    ...
    let () =
      if compute_jacobian then
        ...
        let () =
          export_jac ~propagate_constants logger logger_err compil network sorted_rules_and_decl
        in
        ()
    in
    let () = Format.printf "\t -observables @." in
