...
let annotate_dropped_agent
    ~new_syntax sigs links_annot (agent_name, _ as ag_ty) intf counts =
...
let annotate_edit_agent
    ~new_syntax ~is_rule sigs ?contact_map (agent_name, _ as ag_ty) links_annot
    intf counts =
  let ag_id = Signature.num_of_agent ag_ty sigs in
  let sign = Signature.get sigs ag_id in
  let arity = Signature.arity sigs ag_id in
      ...
      let mix,(lhs_links_one,lhs_links_two) =
        List.fold_left
          (fun (acc,lannot) ((_,pos as na),sites,modif) ->
             ...
             let ra,lannot' =
               annotate_dropped_agent ~new_syntax sigs lannot na intf counts in
             (ra::acc,lannot'))
      ...
      (fun (lannot,acc,news) (ty,sites,modif) ->
         ...
         | None ->
           let a,lannot' = annotate_edit_agent
               ~new_syntax ~is_rule sigs ?contact_map ty lannot intf counts in
           (lannot',a::acc,news)
         | Some Ast.Create ->
           let rannot',x' = annotate_created_agent
         ...
         | Some Ast.Erase ->
           let ra,lannot' = annotate_dropped_agent
               ~new_syntax sigs (fst lannot) ty intf counts in
...
let init_of_ast ~new_syntax sigs tok contact_map ?c = function
  | Ast.INIT_MIX who,pos ->
    Ast.INIT_MIX (mixture_of_ast ~new_syntax sigs ~contact_map pos who ?c),pos
...
let compil_of_ast ~new_syntax overwrite c =
  ...
  let perts',updated_vars =
    List_util.fold_right_map
      (perturbation_of_ast ~new_syntax sigs tok algs contact_map with_counters)
  ...
  sigs,contact_map,tk_nd,algs,updated_vars,
  {
    Ast.variables =
      List_util.mapi
        (fun i (lab,expr) ->
           (lab,alg_expr_of_ast
              ~new_syntax ~max_allowed_var:(pred i)
    ...
    Ast.observables =
      List.map (fun expr ->
          alg_expr_of_ast ~new_syntax sigs tok algs ~c:with_counters expr)
        c.Ast.observables;
    Ast.init =
      List.map (fun (lab,expr,ini) ->
          lab,alg_expr_of_ast ~new_syntax sigs tok algs ~c:with_counters expr,
          init_of_ast ~new_syntax sigs tok contact_map ~c:with_counters ini)
        c.Ast.init;
...
let init_of_ast ~new_syntax sigs contact_map ?c tok algs inits =
  List.map (fun (lab,expr,ini) ->
      lab,alg_expr_of_ast ~new_syntax sigs tok algs expr,
      init_of_ast ~new_syntax sigs tok contact_map ?c ini)
    inits
